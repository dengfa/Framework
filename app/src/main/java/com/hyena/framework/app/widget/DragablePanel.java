...
public class DragablePanel extends RelativeLayout {
	...
	public boolean onInterceptTouchEvent(MotionEvent ev) {
		...
		case MotionEvent.ACTION_DOWN: {
			...
			mDownX = ev.getX();
			mDownY = ev.getY();
			int padding = UIUtils.dip2px(getContext(), PADDING);
	...
	public boolean onTouchEvent(MotionEvent event) {
		...
		case MotionEvent.ACTION_MOVE: {
			...
			if (Math.abs(disX) > UIUtils.dip2px(getContext(), 2)
					&& Math.abs(disY) > UIUtils.dip2px(getContext(), 2)) {
				mMoving = true;
...
//		scale.start();
	...
	private void stickBorder() {
		...
		toLeft = left < (getWidth() - getDragView().getWidth()) / 2;
		ViewPropertyAnimator
				.animate(getDragView())
				.setInterpolator(new AccelerateDecelerateInterpolator())
				.setDuration(200)
				.translationX(
						toLeft ? -getDragView().getLeft() + UIUtils.dip2px(getContext(), 6)
								: getWidth() - getDragView().getLeft()
										- getDragView().getWidth()
										- UIUtils.dip2px(getContext(), 6))
					...
					public void onAnimationEnd(Animator animation) {
						...
						if (toLeft) {
							params.rightMargin = getWidth()
									- getDragView().getWidth() - UIUtils.dip2px(getContext(), 6);
						} else {
							params.rightMargin = UIUtils.dip2px(getContext(), 6);
						}
						requestLayout();
					}
